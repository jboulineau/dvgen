    ...
    public class TSqlObfuscatingFormatter : ISqlTreeFormatter
    {
        ...
        private string GetCaseRandomized(string outputKeyword)
        {
            ...
            for (int i = 0; i < keywordCharArray.Length; i++)
            {
                ...
                keywordCharArray[i] = _currentlyUppercase ? keywordCharArray[i] = Char.ToUpperInvariant(keywordCharArray[i]) : keywordCharArray[i] = Char.ToLowerInvariant(keywordCharArray[i]);
